...
diceOptions.forEach((option, index) => {
    ...
    let dice;
    switch(type) {
        case 'd4': dice = new DiceD4({backColor: '#ffffff'}); break;
        case 'd6': dice = new DiceD6({backColor: '#ffffff'}); break;
        case 'd8': dice = new DiceD8({backColor: '#ffffff'}); break;
        case 'd10': dice = new DiceD10({backColor: '#ffffff'}); break;
        case 'd12': dice = new DiceD12({backColor: '#ffffff'}); break;
        default: break;
    }

    if (dice) {
        // TODO: Update the material. https://threejs.org/examples/#webgl_materials_physical_clearcoat
        dice.getObject().position.set(
            index*200 + 100,
...
cameraControls.setLookAt(
    ...
    diceArray.length * 100, 100, 100, false);
